...
PVUT is a fictional institution created specifically as the subject of this engineering project. Every module, capacity constraint, and workflow is modeled as if it belonged to a real university, so that the resulting architecture is genuinely production viable, not a toy simplification.
...
Modules never reach into another module's data directly. They communicate through events and generic, source agnostic contracts (`ChargeExpected`, `CLEARANCE_UPDATED`). A new department can plug into Finance or Graduation next year without either module's code changing.
...
- ✅ Architecture Phase (Sprint 0 to 2): **Complete.** Module boundaries, ownership map, ERD, and a full day production stress simulation all signed off.
...
Full module detail, every stress test finding, and the reasoning behind each architectural decision live in `docs/architecture-decisions/`.
